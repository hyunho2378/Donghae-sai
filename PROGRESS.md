...
# P5 무코 브랜드 페이지와 잔여 정리 (2026-08-24)

## 단계 0. 무코 브랜드 페이지 신설

- AboutPage 를 브랜드 소개 페이지로 재구성(라우트 /about 유지). 표준 와이드 컨테이너로 좌우 여백 채움
- 섹션: 히어로(태그라인 머무는 여행 이어지는 동해, 동해=accent/동해=primary 색분리) → 이름의 뜻(사이 = 장소와 시간과 사람 잇기) → 선라이즈 링크 심볼(favicon.svg + 일출 물결 양끝 점 의미) → 캐릭터 무코(muko-main.png 정면 + 소개) → 브랜드 색(동해 블루/무코 레드 스와치, 스와치는 토큰, 표기 hex 는 안내 텍스트) → CTA
- RevealOnScroll 섹션 등장. 회색 테두리 없음(shadow-card). 동해 블루 + 무코 레드 구성

## 단계 1. 무코 배치 자리

- 챗봇 히어로 인트로 헤드라인 위에 무코 아바타 슬롯 추가. muko-main.png, 부재 시 Sparkles 아이콘 폴백. loading lazy(홈 첫 렌더 비차단)
- 브랜드 페이지 무코 정면 대형 배치(부재 시 accent-soft 블록 + 무코 텍스트)
- 상세 ScatterIllust 슬롯과 스토리 첫 스팟 블롭은 빈 채 유지(흩뿌림용 소형 애셋 별도 필요. muko-main 은 단일 정면이라 부적합)

## 단계 2. 챗봇 화살표 복귀

- 전송 버튼 ArrowDown → ArrowUp(P1 변경 되돌림). 다른 챗봇 요소는 미변경

## 단계 3. 초이스 텍스트와 문구

- 카드 텍스트 위치 통일: desc line-clamp-2 + min-h-[44px] 로 2줄 고정해 제목 높이 정렬
- 가독성: 전체 오버레이 bg-black/45 + 하단 절반 bg-black/35 솔리드 겹침(그라데이션 금지 준수). desc 색 white/90→white
- 밤이 목적지 → 야간 관광(#야간관광)

## 단계 4. 패스 문구와 여백

- MembershipPage 추천 플랜 배지 주력 → 추천
- 컨테이너 감사: 콘텐츠 페이지는 표준 max-w-[1400px] 2xl:1600 + px 규격 사용 확인. narrow 페이지(Auth 440 로그인, CheckoutComplete 600 완료, Privacy/Journal/Story 720 아티클)는 콘텐츠 성격상 의도된 폭. AboutPage 를 와이드로 재구성해 빈 여백 해소

## 검증

- 밤이 목적지 0, 야간 관광 존재, 주력 배지 0(데이터 산문 주력 상품은 무관), 화살표 ArrowUp, about 브랜드 키워드 존재
- vite build 통과
...
# R2 개별 수정 묶음 (2026-08-24)

R1 위에서 위계 외 개별 문제만. R1 이 잡은 굵기/크기는 재손대지 않음.

## 단계 0. 챗봇 무코와 여백과 행간

- 메인 히어로 첫 진입 무코 제거(P5 아바타 롤백). 무코는 대화가 열린 상태 입력창 옆 아이콘 자리에만(fab 성격). 부재 시 Sparkles 폴백
- SovereignChat FAB 버튼 아이콘 MessageCircle → 무코 이미지(부재 시 MessageCircle)
- 대화 여백 축소: 메시지 space-y-6/8 → space-y-4, 스크롤 pb-6 → pb-3, 하단 pb-8/10 → pb-4/6
- AnswerText 문단 space-y-4 → 2.5, 줄간격 leading-relaxed → leading-normal
- 전송 화살표 위(ArrowUp) 유지. 하단 안내 문구 유지

## 단계 1. 입력창 경계와 헤더 로고

- 챗봇 입력창: 연한 하늘색 테두리 제거. bg-white + shadow-card 로 웜 배경 위에 뜨게, 포커스 시 ring-2 ring-primary 민트 링 또렷
- 헤더 로고 h-6/8 → h-5/6 로 축소

## 단계 2. 스토리 제목과 카드 간격

- 제목 이야기 → 동해 스토리
- 카드 그리드 gap-6/8 → gap-x-6/8 gap-y-10/12 로 세로 간격 확대

## 단계 3. 카드 배지 가독성

- StayCard 유형 배지 variant soft(민트 바탕 민트 텍스트) → primary(동해 블루 바탕 흰 텍스트) + semibold. 대비 확보

## 단계 4. 상세 이름 중복과 라이팅

- 히어로 밖 대형 중복 이름(text-56px 타이포 h2) 삭제. 이름은 히어로 안 h1 한 번만. 공유/저장 버튼은 히어로 아래 얇은 바로 정리
- 소개 본문 endSentence 헬퍼로 마침표 없이 끝나면 자연스럽게 마침표로 맺음(데이터 원문 절단 대응)

## 단계 5. 상세 히어로 높이와 스티키

- 히어로 h-[50vw] max-560 → h-[38vw] max-400 으로 낮춤. 본문 시작 mt-10/12 → mt-6/8, 공유 바 pt-5/6
- 결과: 첫 화면에 히어로 + 본문 시작 + 우측 요금 예약 스티키 함께 노출. 겹침 없음

## 단계 6. 결제 로고 크기

- PayLogo max-h-7 → max-h-11, 결제 카드 h-14 → h-20 md h-88px, px-2 → px-4. GTS 처럼 큼직

## 단계 7. 푸터

- 좌측 텍스트 로고 → logo-wordmark.svg 이미지(컬러라 검은 배경에서 동해=코랄 사이=블루 그대로 노출)
- 메뉴 동해 스토리 / 동해 사이 / 굿즈 / 패스. 프로그램은 독립 항목 제거, 동해 사이 하위 서브 링크로 배치
- 법적 고지와 협력 기관은 P2 그대로 유지

## 검증

- 상세 대형 중복 이름 0, 무코 히어로 대화 상태 전용, 스토리 제목 동해 스토리, 푸터 로고 이미지+프로그램 서브
- vite build 통과
