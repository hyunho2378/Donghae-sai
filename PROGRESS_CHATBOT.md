...
## 5차: 스트리밍 바이트 깨짐 근본 수정 (<0xEC> 문제)

**진단 결과.** 증거 `<0xEC><0x90><0xAC>` 는 UTF-8 3바이트(= "쐬")가 글자로 조립되기 전에 화면에 뜬 것.
- 클라 수신부(useSovereignChat send): `decoder.decode(value, {stream:true})` **이미 있음** → 네트워크 청크 경계 문제(A) 아님.
- 서버(chat.js): Ollama를 `stream:true`로 읽고 `parsed.message.content` 를 **그대로 통과**. 문제는 여기.
- **원인(B): gemma 모델이 한글을 바이트 폴백 토큰(리터럴 `<0xEC>` 문자열)으로 흘림.** llama.cpp 계열의 byte-fallback. 스트리밍이라 토큰 단위로 detokenize돼 글자를 못 이룬 바이트가 `<0xNN>` 문자열로 새어 나옴.

**수정(서버 단독, chat.js).** 근본 수정 채택(스트리밍 유지). 차선책(non-streaming)은 안 씀.
- 토큰이 순수 `<0xNN>`(하나 이상)이면 바이트만 모으고 **완성된 UTF-8 글자만** 클라로 보낸다(미완성 꼬리는 pending 유지). 일반 텍스트 토큰이 오면 남은 바이트를 비우고 이어 붙인다. done/스트림 끝에 잔여 flush.
- **가산적·안전**: `<0xNN>` 토큰이 없는 일반 답변은 로직을 안 거치고 그대로 나감 → 무회귀. `**볼드**` 등 마크다운 텍스트 불변 확인.
- self-check 통과: `<0xEC><0x90><0xAC>`→"쐬"(분리/배치 둘 다), "바닷바람을 쐬는 것도 추천드려요" 재구성, 일반 마크다운 불변, 미완성 꼬리 flush.
